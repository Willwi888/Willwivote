...
import { PlayIcon, SpinnerIcon, CheckIcon, PauseIcon, RetryIcon } from './Icons';
...
export const AdminView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  // Use Audio Context for Testing - NOW WITH ERROR & LOADING STATES
  const { playingId, playSong, pause, isPlaying, isLoading, error: audioError } = useAudio();
  ...
  const handleTestPlay = (song: Song) => {
      ...
      // Safety Check
      if (!rawUrl && !song.youtubeId) {
          alert("⚠️ No Audio Link found for this song. Please add a URL first.");
          return;
      }
      ...
      // If active and playing, just pause
      if (playingId === song.id && isPlaying) {
          pause();
          return;
      }

      // If playing same song but it was paused/error, resume/retry
      // Otherwise play new
            ...
            <div className="space-y-8 animate-slide-up">
                ...
                <div className="space-y-2">
                    {localSongs.map(song => {
                        ...
                        // NEW LOGIC FOR LOADING/ERROR STATE
                        const isActive = playingId === song.id;
                        const isThisLoading = isActive && isLoading;
                        const isThisError = isActive && audioError;
                                     ...
                                     <div className="flex items-center gap-3">
                                         <button 
                                            ...
                                            className={`w-8 h-8 flex items-center justify-center rounded-full border transition-all ${
                                                isActive && isPlaying ? 'bg-gold border-gold text-black' : 
                                                isThisError ? 'bg-red-500 border-red-500 text-white' :
                                                'border-white/20 text-gray-400 hover:text-white hover:border-white'
                                            }`}
                                            title={isThisError ? "Playback Failed (Check Link)" : "Test Play"}
                                         >
                                            {isThisLoading ? <SpinnerIcon className="w-3 h-3" /> : 
                                             isThisError ? <span className="text-xs font-bold">!</span> :
                                             isActive && isPlaying ? <PauseIcon className="w-3 h-3" /> : 
